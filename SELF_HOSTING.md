...
Passkey mode requires a compatible secure origin, browser, unlock device, and stable site identity. Encryption first asks the browser to read an existing passkey for this site, including passkeys on a phone/tablet or security key where supported. If no usable passkey is available, the app can create one reusable site passkey named `Encrypted 2D Barcode`. Each code uses a new random salt. The app falls back to passphrase mode if passkey setup or reuse fails.
